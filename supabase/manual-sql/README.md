...
`supabase_update_chitchat_operating_hours.sql`처럼 공간 운영시간을 수동 SQL로 고쳐야 했던 경우도, 이제 관리자 화면(`공간 정보 관리` > 공간 `수정 열기` > `운영시간 수정`)에서 공통 시간 + 휴무 요일 형태로 바로 수정할 수 있습니다. 요일마다 시간이 다른 세밀한 편집이 필요할 때만 수동 SQL을 사용하세요.
...
2. `02_upsert_admins.sql`
   ...
   - 일상적인 관리자 1~2명 추가/제외는 이제 관리자 화면(`관리자 계정` > `관리자 계정 추가` / `비활성`)에서 바로 처리할 수 있습니다. 이 파일은 엑셀 명단을 통째로 반영해야 하는 대규모 개편 시에만 사용하세요.
