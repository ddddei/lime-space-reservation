            ...
            {sessions.map((session) => {
              ...
              return (
                <tr key={session.id} className="border-b border-[#EBF2E7]">
                  ...
                  <td className="px-3 text-[#5B6856]">{meeting?.applicantName ?? "신청자 없음"}</td>
                  <td className="px-3 text-[#5B6856]">{space?.name ?? "공간 없음"}</td>
                  <td className="px-3 text-[#5B6856]">{session.date} {session.startTime}-{session.endTime}</td>
                  <td className="px-3 text-[#5B6856]">{session.sessionIndex}</td>
                  <td className="px-3">
                    <span className="rounded-full bg-[#F1F8EC] px-2 py-1 text-xs font-bold text-[#5F9820]">{session.status}</span>
                  </td>
                  <td className="px-3">
                    <button
                      type="button"
