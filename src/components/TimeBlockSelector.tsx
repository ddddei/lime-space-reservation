...
import { getEffectiveOperatingHoursForDate, getTimeSlots } from "../lib/reservationRules";
...
export function TimeBlockSelector(props: TimeBlockSelectorProps) {
  ...
  const hours = getEffectiveOperatingHoursForDate(props.date, props.operatingHours);
  ...
  return (
    <section className="ui-card rounded-2xl p-4">
      ...
      <div className="grid max-h-96 grid-cols-3 gap-2 overflow-y-auto pr-1 sm:grid-cols-5 lg:grid-cols-8">
        {slots.map((slot) => {
          ...
          return (
            ...
            >
              <span className="block text-lg font-black">{slot.time}</span>
